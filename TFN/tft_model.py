...
class TemporalFusionTransformer(nn.Module):
    ...
    def define_static_covariate_encoders(self, x):
        ...
        # just change the static variables into embeddings
        # length: 6 static variables 
        ...
        # pdb.set_trace()
        # static_embedding: [64, 8*6] = [64, 48]
        static_embedding = torch.cat(embedding_vectors, dim=1)

        # static_encoder: [64, hidden=8] static_weights: [64,1,6]
    ...
    def forward(self, x):
        ...
        _, static_weights, static_context_s, static_context_e, static_context_h, static_context_c = self.define_static_covariate_encoders(x["inputs"])
        ...
        if self.block=='TCN':
            ...
            # print("here")
            encoder_output, _, _ = self.define_lstm_encoder(past_encoder, static_context_h, static_context_c)
